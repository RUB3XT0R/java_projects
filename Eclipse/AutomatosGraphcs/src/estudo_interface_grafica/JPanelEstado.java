...
public class JPanelEstado extends javax.swing.JPanel {
	...
	private boolean isInicial;  //representa se o estado é inicial
    private Estado estado;      //o proprio estado
    private int larguraFlecha;  //variavel auxiliar para a largura flecha que muda de acordo com o estado ser incial ou não.
    ...
    public JPanelEstado( Estado estado, AutomatoGUI janelaPrincipal ) {
        ...
        larguraFlecha = isInicial ? 20 : 0; //se o estado for inicial, a largura será 10, senão 0
        setSize( new Dimension( 50 + larguraFlecha, 50 ) ); //a tamanho do painel é dado pelo tamanho do circulo do estado mais o da flecha
    ...
    if ( getParent() != null ) {
    	//seta a localização do estado enquanto é arrastado
        ...
        if ( getX() < 0 )  x = 0; // não permite que o estado fique fora da região do painel
        
        if ( getY() + getHeight() > getParent().getHeight() )
            y = getParent().getHeight() - getHeight() - 2;
        if ( getY() < 0 )  y = 0; // não permite que o estado fique fora da região do painel
        
        estado.setXCentral(x);
        estado.setYCentral(y);
        setLocation( x, y );

        // coordenadas para desenho das transições enquanto ele é arrastado
        if ( isInicial ) {
    ...
    protected void paintComponent( Graphics g ) {
        ...
        //se for incial, haverá um desenho, o da seta que represneta o estado incial
        if ( isInicial ) {
