...
public final class AutomatoGUI extends JFrame {
	...
	protected void refreshTrans(){
		if(atual==null) return;  //n_o executa se o estado atual for nulo
	...
	public void selecionaEstadoAtual(Estado e) {
		if(e==null) return; //n_o executa se o novo estado for nulo
	...
	private void excluirEstado(){
		if(a.getInicial().equals(atual))return; //n_o deixa excluir se for inicial
	...
	private void novaTransicao(){
	    ...
	    a.newTransicao(atual.getNome(),c+"",estadoDestino); //cria a nova transi__o
	    refresh(); 
	    refreshTrans();
	}
	...
	private void removerTransicao(){
		if(atual==null) return;  //n_o executa se o estado atual for nulo
		if(tblTrans.getSelectedRow()==-1) return; //se n_o houve uma sele__o efetiva, n_o executa
		// deleta a transi__o
