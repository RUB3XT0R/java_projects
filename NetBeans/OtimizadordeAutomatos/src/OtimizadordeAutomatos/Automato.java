package OtimizadordeAutomatos;
	...
	public class Automato {
		...
		public void addEstado(String nome){
			...
			if(estados.contains(e)) return; //verifica se o estado j_ existe
		...
		public void newTransicao(String estadoOrigem, String simbolo, String... estadosDestinos) {
			//verifica se o simbolo _ vazio(E) ou perten_e ao alfabeto, caso contrario n_o adiciona a transi__o
			...
			if (e.transicoes.containsKey(simbolo.charAt(0))){
				...
				//caso n_o exista, uma lista de estados é criada e inserida a nova transi__o
			   ...
			   for(char chave : q.transicoes.keySet()){  
				 ...
				 if(transNovo.size()>1){                      
				   ...
				   for(Estado destino : transNovo){
					 if (destino.isFinal()) Qnovo.setFinal(true); //basta um dos estados serem finais para o novo estado tambem ser.
					 for(char c : destino.transicoes.keySet()) 
					   for(Estado est : destino.transicoes.get(c)) 
		...
		private void elEInac(Estado atual){
			atual.setConsideracao(true); //todo estado a ser analizado _ acessivel, pois teve referencia de outro que chamou a fun__o
			...
			for(char c : atual.transicoes.keySet()){
				...
				for(Estado e : lista) elEInac(e); //cada estado dessa transi__o (um apenas, devido ao algoritmo anterior) ser_ marcado e analizado
			}
		}
		private void eliminarEstadosInuteis(){
			for(Estado e : estados) //com quase o mesmo principio do anterior, a itera__o varre e procura apenas os estados finais
				if(e.isFinal()) elEInut(e); //esses n_o considerados uteis, e um itera__o _ feita para se decobrir os estados que levam a eles
		...
		private void elEInut(Estado atual){
			for(Estado e : estados){ //percorre os estados em busca de refrencias ao estado atual.
			 if(e.equals(atual) || e.isConsideracao()) continue; //despreza se for o atual ou j_ considerado
		...
		private void deletarEstadosInvalidos(){
			...
			for(int i=0; i<estados.size(); i++){
				...
				if(e.isConsideracao()) continue; //se for considerado util ou acessivel, ignora as instrutl_es e vai ao proximo;
				for(Estado ee : estados) //inicia-se a busca, em cada estado por referencias/transsi__es que o contenha
					for (char c : ee.transicoes.keySet())
						//cas encontre alguma transi__o a esse estado a ser deletado, ela _ removida
		...
		public void deletarEstado(String nome){
			for(int i=0; i<estados.size(); i++){
				Estado e = estados.get(i); //captura o estado atual
				if(!e.getNome().equals(nome)) continue; //se n_o for o estado ignora tudo
				for(Estado ee : estados) //inicia-se a busca, em cada estado por referencias/transsi__es que o contenha
					for (char c : ee.transicoes.keySet())
						//cas encontre alguma transi__o a esse estado a ser deletado, ela _ removida
						if (ee.transicoes.get(c).contains(e)) ee.transicoes.get(c).remove(e); 
