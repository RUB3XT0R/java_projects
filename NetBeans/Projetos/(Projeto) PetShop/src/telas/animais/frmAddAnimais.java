...
public class frmAddAnimais extends javax.swing.JFrame {

    private AnimalDao dao;
    private List<Animal> animais;

    public frmAddAnimais() {
        initComponents();
    ...
    public frmAddAnimais(AnimalDao dao) {
        this();
        this.dao=dao;
    }
    ...
    private void btnokActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnokActionPerformed
        ...
        animal.setTipo((AnimalTipo) cbtipo.getSelectedItem());
        animal.setIdade(Integer.parseInt(txtidade.getText()));
        animal.setDono( (Cliente) cbdono.getSelectedItem());

        try {
            dao.addAnimal(animal);
            Ferramentas.showMensagem("Operação realizada com sucesso.","Atenção");
        } catch (SQLException ex) {
            Ferramentas.showErro("Erro, não foi possível adicionar o animal.","ERRO!!");
        }

        Ferramentas.limparCampos(txtnome,txtraca,txtidade);
        cbtipo.setSelectedIndex(0);
        cbdono.setSelectedIndex(0);
        txtnome.requestFocus();
