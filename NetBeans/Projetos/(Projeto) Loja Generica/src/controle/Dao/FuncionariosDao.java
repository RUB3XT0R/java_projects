...
public class FuncionariosDao extends Dao{
    ...
    public int addFuncionario(Funcionario f) throws SQLException{
        ...
        executeCommand(
                "insert into funcionarios (id,cpf,nome,email,endereco,cidade,estado,telefone,salario,cargo,faturamensal,foto)"
                + "values(?,?,?,?,?,?,?,?,?,?,?,?)",
                f.getId(),f.getCpf(),f.getNome(),f.getEmail(),f.getEndereco(),f.getCidade(),f.getEstado(),f.getTelefone(),f.getSalario(),f.getCargo(),f.getFaturamentoM(),f.getFoto()
        );
        return f.getId();
    }
    ...
    public void updateFuncionario(Funcionario f) throws SQLException{
        executeCommand(
                "update funcionarios set cpf=?,nome=?,email=?,endereco=?,cidade=?,estado=?,telefone=?,salario=?,cargo=?,faturamensal=?,foto=? where id=?",
                f.getCpf(),f.getNome(),f.getEmail(),f.getEndereco(),f.getCidade(),f.getEstado(),f.getTelefone(),f.getSalario(),f.getCargo(),f.getFaturamentoM(),f.getFoto(),f.getId()
        );
    }
