...
public class EditCombosGUI extends javax.swing.JFrame {
   ...
   OperacaoTipo operacao;
    ...
    public EditCombosGUI() {
        initComponents();
        this.setLocationRelativeTo(null);
        this.atual = new Combo();
        this.operacao = OperacaoTipo.inativo;
    }

    public EditCombosGUI(CombosGUI gui,ComboDao dao,Combo ... atual) {
        initComponents();
        this.setLocationRelativeTo(null);
        ...
        if(atual.length>0 && atual!=null){
            ...
            Ferramentas.montaJTable(jTable1,this.atual.getLanches(),ComboItens.class);
            txtpreco.setText(String.valueOf(dao.getTotalDoCombo(this.atual)));
        }else{
            this.atual = new Combo();
            try {
                this.atual.setId(dao.getNextKey());
            } catch (SQLException ex) {}
            this.operacao = OperacaoTipo.novo;
            txtpreco.setText("0.00");
            this.atual.setLanches(new LinkedList<ComboItens>());
        }
        this.dao=dao;
        this.gui=gui;
    }
    ...
    public void refreshTable(){
        Ferramentas.montaJTable(jTable1, atual.getLanches(),ComboItens.class);
        float total = 0f;
        for(ComboItens item : atual.getLanches()){
            total+=item.getQtde()*item.getLanche().getPreco();
        }
    ...
    private void initComponents() {
        ...
        jLabel7.setText("Lanches adicionados:");
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
        new AddLancheGUI(this,dao,atual,atual.getLanches()).setVisible(true);
    ...
    private void btnupMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnupMouseClicked
        ...
        new AddLancheGUI(this,dao,atual,atual.getLanches(),atual.getLanches().get(jTable1.getSelectedRow())).setVisible(true);
    ...
    private void btnexcluirMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnexcluirMouseClicked
        ...
        if (Ferramentas.showPergunta("Realmente deseja excluir esse item?", "Atenção")) {
            ComboItens item = atual.getLanches().get(jTable1.getSelectedRow());
            try {
                dao.removeComboItem(item.getId());
                atual.getLanches().remove(item);
                refreshTable();
            } catch (SQLException ex) {
    ...
    private void btnokMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnokMouseClicked
        ...
        if(atual.getLanches().isEmpty() || atual.getLanches().size()<2){
            Ferramentas.showErro("Para cadastrar o novo combo é preciso pelo menos dois itens.","Erro.");
            return;
        }
        ...
        try {
            if (operacao == OperacaoTipo.alterar) {
                dao.updateCombo(atual, false);
                System.out.println("update");
            } else if (operacao == OperacaoTipo.novo) {
                dao.insertCombo(atual, false);
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new EditCombosGUI().setVisible(true);
            }
        });
    }
